...
export default function TableSortAndSelection(props) {
  ...
  const fetchData = () => {
    ...
    service.post('/api/phones/getSellersPhonesAndComments')
      .then(({ data }) => {
        setRows(data)
      })
      .catch(e => {
        alert(e.message || 'fetch data fail')
      })
              ...
              .map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td>{row.reviewer.firstname} {row.reviewer.lastname}</td>
                    <td>{row.rating}</td>
                    <td>{row.comment}</td>
                    <td>
                      <CellDisplay phoneId={row.phoneId} hidden={row.hidden} reviewId={row._id} />
                    </td>
                  </tr>
                );
              })}
